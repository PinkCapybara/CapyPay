...
const domain = process.env.NEXT_PUBLIC_API_URL || "random";
...
export const optimisticBalanceUpdateAtom = atom(
  ...
  (
    get,
    set,
    updateFn: (current: { amount: number; locked: number }) => {
      amount: number;
      locked: number;
    },
  ) => {
